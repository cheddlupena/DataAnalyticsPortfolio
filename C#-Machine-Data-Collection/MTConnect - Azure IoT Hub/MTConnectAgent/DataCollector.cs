...
namespace MTConnectAgent
{
    ...
    class DataCollector
    {
        ...
        void client_DataItemsChanged(object sender, EventArgs e)
        {
            ...
            Console.WriteLine("Sending "+msg.BodyStream.Length.ToString()+" bytes of CNC data to the Cloud...");
        ...
        void client_DataItemChanged(object sender, DataItemChangedEventArgs e)
        {
            ...
            String te = combined.ToString("MM/dd/yyyy HH:mm:ss");
            ...
            if (!string.IsNullOrEmpty(e.DataItem.Name))
                sbMsg.AppendLine(string.Format("{0},{1},{2},{3},{4},{5},{6},{7}", sessionId, Guid.NewGuid().ToString(), machineName,
                    e.DataItem.Name, e.DataItem.CurrentSample.Value, e.DataItem.CurrentSample.Sequence, te, rowCount));
            Console.WriteLine("Date/Time: " + te + " Value: " + e.DataItem.CurrentSample.Value);
